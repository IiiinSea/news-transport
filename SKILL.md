...
## 核心功能
- 🎯 **智能触发**：自动识别用户的新闻查询请求，只需说「查找XX新闻」即可触发全流程
- 🔍 **多平台采集**：支持百度、新浪、腾讯等多个新闻源，全网搜集相关内容
- 🧠 **智能处理**：自动去重、摘要提取、来源分类
- 📊 **汇总统计**：生成新闻概览报告，包含来源分布、数量统计
- ✉️ **精美邮件推送**：自动生成现代化响应式 HTML 简报，发送到指定邮箱
- 💾 **本地备份**：自动保存历史新闻简报，方便查阅
- ⏰ **定时任务**：支持定时执行，实现每日/每小时自动推送
- 🌐 **可扩展架构**：支持自定义新闻源、输出渠道、处理逻辑
